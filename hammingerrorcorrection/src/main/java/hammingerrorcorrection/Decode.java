...
public class Decode {
    ...
    public static byte[] decoder(final byte[] dataToDecode) {
        ...
        for (final byte data : dataToDecode) {
            ...
            for (byte i = 0; i < 4; i++) {
                outData = BitGetterSetter.setBitAt(outData, bitCount, bits[bitCount % 4]);
                bitCount++;
            }
            if (bitCount == 8) {
    ...
    private static int[] decodeData(byte data) {
        int p1 = BitGetterSetter.getBitAt(data, 0);
        int p2 = BitGetterSetter.getBitAt(data, 2 - 1);
        int p4 = BitGetterSetter.getBitAt(data, 4 - 1);
        int a = BitGetterSetter.getBitAt(data, 3 - 1);
        int b = BitGetterSetter.getBitAt(data, 5 - 1);
        int c = BitGetterSetter.getBitAt(data, 6 - 1);
        int d = BitGetterSetter.getBitAt(data, 7 - 1);
        boolean c1 = (p1 == (a ^ b ^ d));
        boolean c2 = (p2 == (a ^ c ^ d));
        boolean c4 = (p4 == (b ^ c ^ d));
